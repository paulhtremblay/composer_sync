...
def list_blobs(
        ...
        bucket_name:str, 
        prefix:str, 
        delimiter: Optional[str] = None) -> google.api_core.page_iterator.HTTPIterator :
...
def _get_local_d(ignore_patterns:typing.List[str])-> dict:
    d = {}
    for x in walk_tree(the_dir = 'dags', ignore = ignore_patterns):
...
def _get_storage_d(
        bucket_name:str, 
        storage_client:google.cloud.storage.client.Client)-> dict:
    d = {}
...
def get_not_in_local_changed_local(
        ...
        ignore_patterns:list)->typing.Tuple[list, list]:
    not_found_local = []
    diff = []
...
def sync_bucket(
        ...
        verbosity:int = 0):
    ignore_patterns = _get_ignore_patterns(path = ignore_path)
    storage_client = storage.Client(project = project )
    storage_dict = _get_storage_d(bucket_name = bucket_name, storage_client = storage_client)
    local_dict = _get_local_d(ignore_patterns = ignore_patterns)
    not_found_local, diff = get_not_in_local_changed_local(storage_dict = storage_dict,
    ...
    upload_files_to_bucket(storage_client = storage_client,
                           ...
                           verbosity = verbosity)
    return not_found_local, diff + not_found_local
